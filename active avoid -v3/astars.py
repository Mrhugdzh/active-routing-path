...
class AGV(object):
    def __init__(self, x, y, no, tasks, destination):
        ...
        self.path_no = 0
        self.mapList = []
        self.agvNowAction = []  # action_path的备份
        self.task_no = 0
        self.initPosition = self.start  # AGV的起始位置
        self.nowTask = False
        self.prePosition = self.start
        self.agvAllDo = False
        self.color = [(no * 90+180) % 255, (no * 65+90) % 255, (no * 44+270) % 255]
    ...
    def doOne(self, startNode, targetNode, agv, agv_list):
        ...
        if len(self.openList) == 0:
            ...
            if tem:
                ...
                result_path.reverse()
                results = result_path
                agv.result_paths[0] = copy.deepcopy(results)
                agv.finished_work()
...
def changePointToActions(agv):
    """将路径转化为agv行进方向"""
    path = agv.result_paths[0]
...
def initAgvMapList(agv_list, map):
    for agv in agv_list:
        agv.mapList.append(copy.deepcopy(map))
    return agv_list
...
def squeeze(agvList, n):
    """确定前瞻n步的行进内容"""
    for agv in agvList:
        if agv.agvAllDo == False:
            agv.agvNowAction = copy.deepcopy(agv.action_path)
            if len(agv.action_path[0]) >= n:
                del agv.action_path[0][n:]
                # print(agv.action_path[0])


def getMapList(agvList):
    import copy
    map0 = agvList[0].mapList[0]  # 获取初始地图
    # 如果一个AGV剩余步数不够前瞻n步，那么在完成这个任务之后，开始前瞻下一个任务
    # 初始操作
    ...
    for agv in agvList:
        if agv.agvAllDo == False:
            if agv.nowTask:
                ...
                agv.now_position = nowPosition  # 改变AGV的现在的位置
                ...
                continue
            try:
                agv.action_path[agv.task_no][0]
            except IndexError:
                print('当前任务完成，转移到下一个任务的前瞻')
                agv.nowTask = True
            if agv.nowTask:
                nowPosition = agv.now_position  # 获得AGV的起始位置
                agv.now_position = nowPosition  # 改变AGV的现在的位置
                ...
                map[nowPosition[0]][nowPosition[1]].mapAction = None
                continue
            agv.now_position = agv.initPosition
    ...
    for i in range(n-1):
        ...
        for agv in agvList:
            if agv.nowTask:
                nowPosition = agv.now_position  # 获得AGV的起始位置
                agv.now_position = nowPosition  # 改变AGV的现在的位置
                print('在map{}上添加位置{}，{}，为agv{}'.format(i + 1, nowPosition[0], nowPosition[1], agv.no))
                map[nowPosition[0]][nowPosition[1]].mapAgvNo = agv.no
                map[nowPosition[0]][nowPosition[1]].mapAction = None
                continue
            try:
                action = agv.action_path[0][i]
            except IndexError:
                print('agv{}当前任务完成，转移到下一个任务的前瞻'.format(agv.no))
                agv.nowTask = True
            if agv.nowTask:
                nowPosition = agv.now_position  # 获得AGV的起始位置
            ...
            if newPosition[0] <0 or newPosition[0] >=51 or newPosition[1] < 0 or newPosition[1] >= 51:
                print(agv.result_paths)
                print(agv.action_path[0])
                print('agv{},现在位置是{}，动作为{}，新位置为{}'.format(agv.no, agv.now_position, agv.action_path[0][i], newPosition))
            print('在map{}上添加位置{}，{}，为agv{},动作为{}'.format(i + 1, newPosition[0], newPosition[1], agv.no,agv.action_path[0][i]))
            agv.now_position = copy.deepcopy(newPosition)  # 改变AGV的现在的位置
            map[newPosition[0]][newPosition[1]].mapAgvNo = agv.no
            try:
                map[newPosition[0]][newPosition[1]].mapAction = agv.action_path[0][i + 1]

            except IndexError:
                map[newPosition[0]][newPosition[1]].mapAction = None
...
def getInitPosition(agv):
    """获取此AGV的初始位置"""
    mapss = agv.mapList[0]
    for i in range(mapss.shape[0]):
        for j in range(mapss.shape[1]):
            if mapss[i][j].mapAgvNo == agv.no:
                return [i, j]
...
def run(agv1, agv2, agv1Position, agv2Position, map, agvList):
    ...
    gaiAgvInitPosition = getInitPosition(gaiAgv)  # 获得需要重新规划路径的AGV的初始位置
    print('需要重新规划路径的AGV起始位置为', gaiAgvInitPosition)
    # 设置新的地图信息
    ...
    gaiAgv.tasks.append(gaiAgv.result_paths[0][-1])  # 得到此时路径规划的目的地
    # 开始规划AGV路径
    while 1:
    ...
    print('规划AGV路径完成')
    # 将重新规划路径的AGV的路径转为方向
    #print(gaiAgv.result_paths)
    actions = changePointToActions(gaiAgv)
    print('重新规划之后的动作：',actions)
    gaiAgv.action_path[0] = copy.deepcopy(actions)
    print(gaiAgv.action_path[0])
